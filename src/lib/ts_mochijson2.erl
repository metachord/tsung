...
%% @doc Yet another JSON (RFC 4627) library for Erlang. ts_mochijson2 works
%%      with binaries as strings, arrays as lists (without an {array, _})
%%      wrapper and it only knows how to decode UTF-8 (and ASCII).

-module(ts_mochijson2).
...
json_encode(I, _State) when is_integer(I) ->
    ts_mochinum:digits(float(I));
json_encode(F, _State) when is_float(F) ->
    ts_mochinum:digits(F);
...
json_encode_string_unicode([C | Cs], State, Acc) ->
    Acc1 = case C of
               ...
               %% in-line. ts_mochijson2 does not protect you from this, so
...
encoder_utf8_test() ->
    ...
    Enc = ts_mochijson2:encoder([{utf8, true}]),
